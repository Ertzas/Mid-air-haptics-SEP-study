...
namespace ContinuousSpeedExperiment
{
            ...
            try
            {
                if (!File.Exists(_wavFilePath))
                {
                    string[] possiblePaths = new string[]
                    {
                        ...
                        Path.Combine(@"C:\Users\wolf_\OneDrive\HTW\Master\3_Semester\Leen_PC\haptics\leen_2\TcpServer_And_Ultrahaptics\TcpServer_And_Ultrahaptics", _wavFilePath)
                    };
    ...
    public class ContinuousSpeedExperiment
    {
        ...
        public void RunExperiment(int participantId, int startBlock = 1)
        {
            ...
            // 2. SCHREIBE KOMPLETTE CSV AM ANFANG
        ...
        // SCHREIBE KOMPLETTE CSV AM ANFANG
        private void WriteCompleteCSV(int participantId)
        {
            using (var writer = new StreamWriter(_csvFilename))
            {
                ...
                // all trials (timestamp filled during run)
        ...
        private void UpdateTrialTimestamp(TrialData trial)
        {
            try
            {
                ...
                // find row for this trial (TrialNumber + 1 to skip header)
                int lineIndex = trial.TrialNumber; // header is row 0, trial 1 is row 1
        ...
        static void Main(string[] args)
        {
            ...
            emitter.EmissionCallback = (em, interval, deadline) =>
            {
                foreach (var sample in interval)
                {
                    ...
                    // Zeitdifferenz zwischen aufeinanderfolgenden Samples
                    ...
                    // Dummy-Position setzen (muss sein, sonst kein Output)
            ...
            Console.WriteLine("Starte 3-Sekunden Sample-Rate Test...");
            emitter.Start();
            Thread.Sleep(3000);
            emitter.Stop();
            ...
            Console.WriteLine($"  Gesamtdauer:          {totalSeconds:F3} s");
            Console.WriteLine($"  Samples gesamt:       {totalSamples}");
            Console.WriteLine($"  Callbacks gesamt:     {totalCallbacks}");
            Console.WriteLine($"  ─────────────────────────────────────");
            Console.WriteLine($"  Sample-Rate:          {samplesPerSecond:F0} Hz");
            Console.WriteLine($"  Samples pro Callback: {samplesPerCallback:F1}");
