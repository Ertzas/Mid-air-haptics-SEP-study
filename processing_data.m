%% SEP COMPLETE ANALYSIS PIPELINE
%% Processing → Balanced Epoch Selection → Visualization → Peak & WPSS Analysis


clear all; close all; clc;

fprintf('SEP COMPLETE ANALYSIS PIPELINE');
...
%% FIX 4: Electrode list defined ONCE here, used everywhere below
...
fprintf('PART 1: PROCESSING ALL PARTICIPANTS\n');
...
for p = 1:length(PARTICIPANTS)
    ...
    try
        ...
        for block = 1:length(DATA_FILES)
            ...
            %% Step 1: Apply artifact rejection to epochs
            ...
            %% Step 2: Compute global trial_info indices for clean epochs
            ...
            %% Step 3: Keep only epochs where trial_info row exists (safety check)
            ...
            %% Step 4: Collect matching trial_info rows
...
fprintf('PART 2: BALANCED EPOCH SELECTION\n');
...
for e = 1:N_ELECTRODES
    ...
    for s = 1:n_speeds
        ...
        SEM.(el)(s,:) = std(vp_avg, 0, 1) / sqrt(n_participants);   % FIX 2 ✓
    end
end
...
%% ─── Figure 2: Individual overlay (FIX 3: uses balanced averages) ───
...
for e = 1:N_ELECTRODES
    ...
    for s = 1:n_speeds
        ...
        total_content = n_speeds*pw + (n_speeds-1)*hgap + 0.008 + 0.012;  
        x_start = (1 - total_content) / 2;  
        xl = x_start + (s-1)*(pw+hgap);      
        ax_hm(s) = axes('Position', [xl, yb, pw, ph]); 
...
%% ─── Figure 7: Participant-Average Heatmap + Grand Average Overlay ────

fprintf('Creating Figure 7: Participant-Heatmap + Grand-Average overlay...\n');

%% ── Compute speed-stratified participant averages ──────────────────────
PA_matrix = struct();
for e = 1:N_ELECTRODES
    el = ELECTRODES{e};
    PA_matrix.(el) = zeros(n_participants, n_samples, n_speeds);
    for s = 1:n_speeds
        speed = unique_speeds(s);
        for pp = 1:n_participants
            pid     = successful_participants{pp};
            pp_mask = strcmp(BALANCED_participant_labels, pid);
            sp_mask = [BALANCED_trial_info.speed] == speed;
            PA_matrix.(el)(pp, :, s) = mean(ELECTRODE_DATA.(el)(pp_mask & sp_mask, :), 1);
        end
    end
end
...
pid_labels_fig7 = cell(1, n_participants);
for pp = 1:n_participants
    pid_labels_fig7{pp} = successful_participants{pp};
end

for e = 1:N_ELECTRODES
    el = ELECTRODES{e};

    fig_ph = figure('Position', [50 + (e-1)*60, 50, 2200, 450], 'Color', 'white');

    pw   = 0.13;
    hgap = 0.03;
    ph   = 0.62;
    yb   = 0.18;
    total_content = n_speeds * pw + (n_speeds - 1) * hgap + 0.008 + 0.015;
    x_start = (1 - total_content) / 2;

    ax_arr = gobjects(1, n_speeds);

    for s = 1:n_speeds
        xl = x_start + (s - 1) * (pw + hgap);
        ax_arr(s) = axes('Position', [xl, yb, pw, ph]); 

        pa_data = PA_matrix.(el)(:, :, s);
        imagesc(time_ms, 1:n_participants, pa_data);
        hold on;

        ga_trace = GA.(el)(s, :);
        y_centre = (n_participants + 1) / 2;
        y_half   = n_participants / 2;
        ga_y     = y_centre + (ga_trace / clim_hm(2)) * y_half * 0.8;

        plot(time_ms, ga_y, 'w-', 'LineWidth', 2.5);

        yline(y_centre, ':w', 'LineWidth', 0.8, 'Alpha', 0.6);
        ...
        colormap(ax_arr(s), jet);
        caxis(clim_hm);
        set(gca, 'YDir', 'normal');
        set(gca, 'YTick', 1:n_participants, 'YTickLabel', pid_labels_fig7);
        xlabel('Time (ms)', 'FontSize', 11, 'FontWeight', 'bold');
        if s == 1
            ylabel('Participant', 'FontSize', 11, 'FontWeight', 'bold');
        else
            set(gca, 'YTickLabel', []);
        end
        title(sprintf('%.1f m/s', unique_speeds(s)), ...
              'FontSize', 12, 'FontWeight', 'bold');
        set(gca, 'FontSize', 10);
    end

    p_last = get(ax_arr(n_speeds), 'Position');
    cb = colorbar('Position', [p_last(1) + p_last(3) + 0.008, yb, 0.015, ph]);
    ylabel(cb, 'Amplitude (µV)', 'FontSize', 11);

    sgtitle(sprintf('%s – Participant Averages + Grand Average (white) | Ref: %s', ...
            el, REFERENCE_METHOD), 'FontSize', 14, 'FontWeight', 'bold');

    fname_ph = sprintf('ParticipantHeatmap_%s_%deps_%s.png', ...
                        el, N_EPOCHS_PER_SPEED, REFERENCE_METHOD);
...
fprintf('PART 4: COMPREHENSIVE PEAK ANALYSIS\n');
...
for s = 1:n_speeds
    ...
    fprintf('│ %5.1f  │  %6.2f  │  %6.2f  │  %6.2f  │  %6.2f  │  %6.2f  │\n', ...
            unique_speeds(s), ...
...
fprintf('┌────────┬──────────┬──────────┬──────────┐\n');
fprintf('│ Speed  │ P1 WPSS  │ N1 WPSS  │ P2 WPSS  │\n');
fprintf('├────────┼──────────┼──────────┼──────────┤\n');
for s = 1:n_speeds
    fprintf('│ %5.1f  │  %6.3f  │  %6.3f  │  %6.3f  │\n', ...
            unique_speeds(s), wpss_p1(s), wpss_n1(s), wpss_p2(s));
end
fprintf('├────────┼──────────┼──────────┼──────────┤\n');
fprintf('│  Mean  │  %6.3f  │  %6.3f  │  %6.3f  │\n', ...
